    ...
    try: 
            ...
            for i in range(len(list2)):                                                 #this part of code mainly focuses on printing each column downwards
                for j in range(len(list2[i])):                                          #with putting the mark ":" after a number of spaces to align with header
                    if j==(len(l)-1):                                                   #for that purpose, the code print(list2[i][j]," "*((len(str(l[j]))-2)-len(str(list2[i][j]))),':',end='')
                        ...
                        print(list2[i][j]," "*((len((l[j]))-2)-len(str(list2[i][j]))),':',end='')
        ...
        def DTABLE():
            ...
            if x.lower()!='users':
                ...
                DTABLE()
        def usecreate1():
            c.execute("create table movie(SLNO integer not NULL primary key,list_of_movies varchar(80), Genre_of_movie varchar(20), date_released date, rating_of_movie float, type_of_movie varchar(20), Language_of_movie varchar(20))")
